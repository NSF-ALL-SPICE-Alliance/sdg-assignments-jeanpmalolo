...
```{r}
rf_neonat <- randomForest(normalized_score_sdg3_neonat ~ .,
...
```{r}
importance_df <- as.data.frame(rf_neonat$importance)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Neonatal Mortality",
       subtitle = "Top 10",
       y = "SDG Indicator",
...
```{r}
pdp::partial(rf_neonat, pred.var = "normalized_score_sdg3_u5mort", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_neonat, pred.var = c("normalized_score_sdg3_u5mort", "normalized_score_sdg3_lifee"))
# Default PDP
plotPartial(pd)
```
