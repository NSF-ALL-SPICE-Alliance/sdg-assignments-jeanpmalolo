...
```{r}
ggplot(data = sdr_data, aes(x= goal_1_score, y = goal_2_score, color = regions_used_for_the_sdr)) +
  geom_point()
```


```{r}
ggplot(data = sdr_data, aes(x= goal_1_score, y = goal_2_score, color = goal_6_score)) +
  geom_point()
```
Interpretation: The chart indicates that the countries with low SDG 6 (clean water and sanitation) scores place lower in SDG 1 (no poverty) & SDG 2 (zero hunger). And according to the chart above this one, using just the 2 scores, these countries with the low SDG scores are mostly the sub-Saharan Africa region and oceanic region. 
...
```{r}
ggplot(mena_sdr_data, aes(x= goal_1_score, y = country)) +
  geom_bar(stat = "identity") 
```
Interpretation: This chart shows that our y-axis consists of a list of the countries in the MENA region and the x-axis shows a range of how well these countries scored in SDG 1 (no poverty). As we can see, Saudi Arabia and United Arab Emirates scored the highest at about 100 and the Republic of Yemen scored the lowest. 
...
```{r}
ggplot(sdr_data, aes(x = goal_2_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
Interpretation: This chart shows that countries with the highest range scores in SDG goal 2 are the East Europe and Central Asia regions, while the Sub-Saharan African region has countries with some of the lowest scores in this particular goal. 
