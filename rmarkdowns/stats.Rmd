...
```{r}
 sdrgc <- ggplot(sdr_data, aes(x = goal_8_score, 
                           y = goal_12_score, 
                           label = country)) +
  geom_point() +
  geom_smooth() +
...
- Edit the code chunk above to plot a linear regression of 2 different sdg scores 
  ...
  - in 4-6 sentences, come up with a hypotheses for the positive or negative correlation between the 2 goals
      Answer: As shown above, SDG 8 (decent work & economic growth) and SDG 12 (responsible consumption and production) show a negative correlation. This may be because it is difficult to be responsible consumers and see economic growth. Countries who may be seeing economic growth are countries who don't seem to be conscious about mass production and consumption and countries who are conscious of this issue find it hard to build their economy because their limited production and consumption. 
      
   - are there any points that don't follow the trend (outliers)? which country(s) are outliers?
    Answer: There are a few outliers such as Norway with a very low sdg 12 score and significantly high sdg 8 score.